...
const mockConversations = [
  { id: 'c1', name: 'Amit Sharma', last: 'Can you update the spec?', time: '2h', unread: 2 },
  { id: 'c2', name: 'Riya Patel', last: "Looks great — I'll review today.", time: '1d', unread: 0 },
  { id: 'c3', name: 'Vikas Kumar', last: 'Sent you the files', time: '3d', unread: 0 },
];
...
const Messages = () => {
  ...
  return (
    <main className="pt-20 pb-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto flex flex-col h-[calc(100vh-6rem)] min-h-[540px]">
        {/* Header */}
        ...
        <div ref={containerRef} className="flex flex-col md:flex-row gap-0 flex-1 min-h-0">
          ...
          <div className={`flex min-h-0 ${selected ? 'hidden md:flex' : 'flex'}`} style={{ ...(isDesktop ? { width: leftWidth } : { width: '100%' }) }}>
            <Card
              hover={false}
              className="relative p-3 sm:p-4 flex flex-col w-full h-80 md:h-full min-h-0"
            ...
            >
              ...
              {/* Chat list */}
              <div className="divide-y overflow-y-auto min-h-0 scrollbar-thin" style={{WebkitOverflowScrolling: 'touch'}}>
                {convs.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => setSelected(c.id)}
                    className={`w-full text-left py-3 px-2 flex items-center gap-3 ${
                      selected === c.id
              ...
              {/* floating add button at bottom-right of conversations card */}
              <div className="md:absolute fixed right-4 bottom-6 md:right-3 md:bottom-3">
                <Button
                  variant="primary"
                  aria-label="New conversation"
                  className="w-14 h-14 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shadow-2xl bg-primary-600 hover:bg-primary-700"
                >
                  <Plus size={24} />
                </Button>
              </div>
                  ...
                  <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3 space-y-3 bg-gray-50 min-h-0 scrollbar-thin" style={{WebkitOverflowScrolling: 'touch'}}>
                    ...
                    {messages.map((m) => (
                      ...
                      >
                            ...
                            <div
                          className={`max-w-[75%] sm:max-w-[70%] px-3 py-2 rounded-2xl text-sm shadow-sm ${
                            m.fromMe
